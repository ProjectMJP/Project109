...
public class LobbyInteractionStand : MonoBehaviour
{
    ...
    public void Interact()
    {
        ...
        {
            RunManager.instance.SetTempStarterKit(_targetStarterKitId);
            Debug.Log($"[Lobby] 시작 무기/로드아웃이 '{_standDisplayName}'({_targetStarterKitId})으로 임시 지정되었습니다.");
            
            // UIManager 연동이나 파티클 연출이 필요하다면 여기에 트리거 가능
        }
        else
        {
            Debug.LogWarning("[Lobby] RunManager instance가 존재하지 않습니다.");
        }
    }
}
